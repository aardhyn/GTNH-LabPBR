...
def ensure_base_texture(source: str, destination: str):
...
def generate_specular(image: Image.Image) -> Image.Image:
    ...
    r = np.minimum(r, 75).astype(np.uint8)

    g = np.full_like(g, 255, dtype=np.uint8)
...
def process_texture(mod_root: str, mod_file: str, output_folder: str):
    path_parts = mod_root.split(ASSETS_DIR)[-1][1:]
    folder_path = os.path.join(output_folder, ASSETS_DIR, path_parts)

    source_texture_path = os.path.join(mod_root, mod_file)
...
def process_jar(jar_path: str, output_folder: str, blacklist: set[str] | None):
...
def get_output_folder(source_folder: str, base_name: str, custom_base: str | None = None) -> str:
    """Returns a new deduplicated output folder path."""
    if custom_base:
        folder = Path(custom_base)
    else:
        folder = Path(source_folder) / "resourcepacks" / base_name
...
def main():
    parser = argparse.ArgumentParser(description="Generate specular maps for Minecraft textures.")
    parser.add_argument("-s", "--source", required=True, help="Path to the Prism Minecraft instance")
    parser.add_argument("-b", "--blacklist", required=False, help="Optional path to blacklist.txt")
    parser.add_argument("-o", "--output", required=False, help="Optional custom output directory")
